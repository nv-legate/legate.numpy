                    ...
                    ncclComm_t* comm)
{
...

  {
    ...
    */
    ...
    LegateCheckCUDA(cudaMemcpy2DAsync(send_right.ptr(0),
                                      sizeof(int64_t),
    ...
    LegateCheckCUDA(cudaMemcpy2DAsync(send_left.ptr(0),
                                      sizeof(int64_t),
                                      segment_diff_2d_scan.ptr(0) + my_sort_rank,
                                      num_sort_ranks * sizeof(int64_t),
                                      sizeof(int64_t),
  ...
  ncclComm_t* comm)
{
  ...
  // This is all2all restricted to one sort row
  {
    ...
    LegateCheckCUDA(cudaMemcpyAsync(send_buffer.ptr(0),
                                    samples.ptr(offset),
    ...
    for (size_t r = 0; r < num_sort_ranks; r++) {
      if (r != my_sort_rank) {
        LegateCheckCUDA(cudaMemcpyAsync(samples.ptr(num_samples_l * r),
  ...
  Buffer<size_t> size_recv_total =
    create_buffer<size_t>(num_sort_ranks, legate::Memory::Z_COPY_MEM);
  {
    LegateCheckCUDA(cudaMemcpy2DAsync(size_send_total.ptr(0),
                                      1 * sizeof(size_t),
                                      ...
                                      stream));
    LegateCheckCUDA(cudaMemcpy2DAsync(size_recv_total.ptr(0),
                                      1 * sizeof(size_t),
